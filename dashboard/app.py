...
def render_health(raw: pd.DataFrame):
    """Boru hattının her halkasını yoklar. Amaç: 'panel boş' dendiğinde
    hangi halkanın koptuğunu tahminle değil ölçümle bulmak."""
    import re
    ...
    rows = [
        ...
        ("Spark consumer (4040)", spark_up,
         ...
         "CONSUMER penceresini yeniden başlat → `.venv\\Scripts\\python consumer\\spark_consumer.py`"),
...
İki seviye var çünkü süreç **ayakta ama işlevsiz** olabilir (bugünün dersi: consumer penceresi
açık görünürken Spark içeride çökmüştü). Port yoklaması "yaşıyor mu", tazelik/akış "işini
yapıyor mu" sorusudur — üretim sistemlerinde buna *liveness* vs *readiness* denir.
""")
...
kaybolur — bugün consumer'ın ilk çöküşünde sebebini bu yüzden göremedik. Başlatma
...
# =====================================================================
# 6) SAĞLIK — boru hattı çalışıyor mu, hatalar nerede?
# =====================================================================
if section == "Sağlık":
    render_health(raw)
